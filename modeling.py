...
class AutoModelForSequenceClassification_RGAT(nn.Module):
    """MelBERT"""

    def __init__(self, args, Model, config, num_labels=2):
        """Initialize the model"""
        super(AutoModelForSequenceClassification_RGAT, self).__init__()
        self.num_labels = num_labels
        self.encoder = Model
        self.config = config
        self.dropout = nn.Dropout(args.drop_ratio)
        ...
        self.dep_embed = nn.Embedding(args.dep_tag_num, args.embedding_dim)
    ...
    def forward(
        ...
        feature = torch.stack([torch.index_select(f, 0, w_i)
                               for f, w_i in zip(sequence_output, word_indexer)])
        '''
        
        neighbor_mask = neighbor_mask/neighbor_mask.sum()
        feature = sequence_output * neighbor_mask.unsqueeze(2)
        #print(feature.size())
        ...
        dep_feature = torch.mul(feature, dep_feature)
        ...
        # Get hidden vectors each from SPV and MIP linear layers
        SPV_hidden = self.SPV_linear(torch.cat([feature, target_output], dim=1))
        MIP_hidden = self.MIP_linear(torch.cat([target_output_2, target_output], dim=1))
